...
public class SampleScripts : MonoBehaviour
{
    ...
    public int gid;
    public SheetLoader<charData> data;
    [SerializeField]public List<charData> Ldata=new List<charData>();
     
    async void Start()
    {
        // 1. 로더 생성
        data = new SheetLoader<charData>(url, gid);
        var tttt=await data.GetDataByString("zombie");
